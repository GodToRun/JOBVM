...
public abstract class OSLoader extends GameManager implements KeyListener {
		...
		try {
			...
			try {
				...
				String sector = vp.getDisk('A').FileList.get(BOOT_SECTOR_NAME).content;
						try {
							ArrayList<Character> curs = new ArrayList<Character>();
							for (String i : sector.split(" ")) {
								curs.add((char)Integer.parseInt(i));
							}
							Object[] refArray = curs.toArray();
							char[] charArray = new char[refArray.length];
							for (int i = 0; i < refArray.length; i++) {
							    charArray[i] = (char)refArray[i];
							}
							cpu.request(charArray);
						}
	...
	public void readSector(char[] com) {
		cpu.request(com);
	}
