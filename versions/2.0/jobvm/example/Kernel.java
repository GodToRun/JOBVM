...
public class Kernel extends OSLoader {
	...
	public void DrawShell(Graphics g) {
		try {
		graphic.setPixel(MouseInfo.getPointerInfo().getLocation().x, MouseInfo.getPointerInfo().getLocation().y, (short)VGA.LIGHTGRAY);
		}
	...
	public void Transfer(String line) {
		...
		/*else if (arg[0].equals("mkdir")) {
			...
			memory.push((byte)Integer.parseInt(arg[1]));
		}
			...
			int index = 0;
			for (byte val : memory.getStackMap()) {
				if (val != -128 && val != 0) {
				graphic.Println(Integer.toHexString(index) + ": " + val);
				}
				index++;
			}
		}
